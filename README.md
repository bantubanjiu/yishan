...
[![Version](https://img.shields.io/badge/version-0.2.7-2563eb)](./版本记录README.md)
...
移山是一个本地优先的轻量 Web Clipper：不走云端服务，不弹复杂表单，浏览器扩展通过 Native Messaging 调用本机 Node Host，直接写入 Obsidian Vault。保存页面剪藏会尽量提取正文 Markdown、下载页面图片并生成单独 Markdown 文档；选中文本、富 Markdown、图片、截图和多标签链接继续写入当天 Inbox，并按同一天同一网址合并到同一个网页分组。

适合高频收集：完整页面、选中文本、网页图片、局部截图，以及当前窗口多个普通标签页。除“保存页面剪藏”外，轻量保存会在当天 Inbox 里保持稳定分组格式：
...
| 保存选中文本 | 选中文本后右键保存；默认尽量以普通文本保存，可在“安全纯文本”和“富 Markdown”模式间切换；同日同网址摘录会挂在同一个网页链接下。 |
...
- 选中文本右键：保存选中文本和页面链接；默认用安全纯文本并尽量以普通文本记录，切换到富 Markdown 后只有在确实包含标题、加粗、列表、链接、引用、代码块或图片等格式时才保留富文本，失败时回退纯文本；同一天同一网址的多次摘录会追加在同一个网页链接下面。
...
### 为什么当天 Inbox 里同一网址只出现一个链接？

这是 v0.2.7 起的稳定格式：网页链接作为 `#### [标题](URL)` 分组标题，下面用 `- HH:mm 类型` 记录每次保存的材料，不再把记录点写成标题。同一天多次保存同一网址的文字、富文本、图片或截图时，会追加在同一个链接下面；普通摘录尽量以文本保存，只有确实有加粗、链接、列表等结构时才保留富 Markdown；当天日记文件名仍按本机本地日期生成。

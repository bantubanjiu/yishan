...
| 保存选中文本 | 选中文本后右键保存；可在“安全纯文本”和“富 Markdown”模式间切换；同日同网址摘录会挂在同一个网页链接下。 |
...
这是 v0.2.5 起的稳定格式：网页链接作为 `## [标题](URL)` 分组标题，下面用 `### HH:mm 类型` 记录每次保存的材料。同一天多次保存同一网址的文字、富文本、图片或截图时，会追加在同一个链接下面；当天日记文件名仍按本机本地日期生成。
